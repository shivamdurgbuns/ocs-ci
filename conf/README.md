...
* `rgw_cert_ca` - URL for the RGW signing CA when external Ceph is **below 8.0**, or as a **fallback** if cephadm CA fetch fails on 8.0+
* For external Ceph **8.0 and newer**, ocs-ci runs ``ceph orch certmgr cert get cephadm_root_ca_cert`` via ``cephadm shell`` on the ``_admin`` node (``get_external_cluster_client("_admin")``, falling back to ``node1``) instead of using ``rgw_cert_ca``, unless that command fails
